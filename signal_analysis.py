...
def spectral_flatness(channel): # Spectral Flatness Geometric mean Power Spectrum / airthematic mean power spectrum
...
def split_output_band(ch_output):
    ...
    print("Indices",len(ch_output))
    for i in range(len(ch_output)):
        #print("ch_output:",ch_output)
        #print("i:",i)
        #print("len:",len(ch_output))
        #if not ch_output.ndim==2:
        #    ch_output = np.reshape((len(ch_output),-1),ch_output))
        #    print(ch_output.shape)



        #print("shape of ch_output:",ch_output.shape)
...
def Slice_wavfile(audio):
    start_time = eval(input("Enter start time in mili seconds: "))
    end_time = eval(input("Enter end time in mili seconds: "))
    cliiped_audio = clip_wav.slice(audio,start_time, end_time)
    return cliiped_audio


def extract_wav_fet(wav_file,output_dir,spectogrpahs=False):
    print("wav_file:",wav_file)
    directory, filename = ntpath.split(wav_file)
    print("directory:",directory)
    print("filename:",filename)

    xlxdirectory = os.path.join(output_dir,'xlxdirectory')
    ...
    #print "Channel Length: {}".format(len(data[:,0]))

    #input_val = raw_input("Do you want to slice wave file [Y/N]:")
    input_val = 'n'
    output_file = wav_file
    if input_val =='y' or input_val =='Y':
        output_file = Slice_wavfile(wavfile)
        print("Clipped file : {}".format(output_file))
    ...
    left_channel = data[:,0]
    print("shape of left channel before downsampling:",left_channel)
    right_channel = data[:,1]
    print("shape of right channel before downsampling:",right_channel.shape)
    print("Warning! Downsampling to 16 KHz . Will be removed in future")
    # Down Sample to 16kHz
    targetsampFreq = 16000
#rahul commenting all the resampling
    #left_channel = libresample(left_channel,sampFreq,targetsampFreq)
    #print("shape of left channel after downsampling:",left_channel.shape)
    #right_channel = libresample(right_channel,sampFreq,targetsampFreq)
    #print("shape of right channel after downsampling:",right_channel.shape)
    ...
    print("shape of  fourier:",fourier.shape)
    real_fourier = np.absolute(fourier).T #transpose
    print("shape of real fourier:",real_fourier.shape)
    ...
    print("shape of left_channel_fourier_sss:",left_channel_fourier_sss.shape)
    right_channel_fourier_sss = right_channel_fourier[0:int(ch_length/2+1)]
    print("shape of right_channel_fourier_sss:",right_channel_fourier_sss.shape)

    # Rationalize for real Power for both channel
    left_channel_fourier_sss[1:-1]  = 2*left_channel_fourier_sss[1:-1]
    print("shape of left_channel_fourier_sss of Power:",left_channel_fourier_sss[1:-1].shape)
    right_channel_fourier_sss[1:-1] = 2*right_channel_fourier_sss[1:-1]
    print("right_channel_fourier_sss:",right_channel_fourier_sss[1:-1])

    # frequency vectorsss
    f = (np.arange(0,ch_length/2+1)/float(ch_length))*float(targetsampFreq)
    f = np.asarray(f[:len(right_channel_fourier_sss)])

    print("shape of frequency vector:",f.shape)
    m=f.shape[0]
    #print("m:",m)

    # Band splitting
    before_transpose=[]
    before_transpose=np.asarray((f,left_channel_fourier_sss,right_channel_fourier_sss))

    #before_transpose = np.vstack(before_transpose)
    print("before_transpose:",before_transpose)
    ch_output=np.transpose(before_transpose)
    ##ch_output = np.transpose(np.asarray((f,left_channel_fourier_sss,right_channel_fourier_sss)))
    np.asarray(ch_output)
    #n=ch_output.shape[-1]
    #print("n:",n)
    #ch_output.reshape(m,n)
    ...
    print ("computing signal Sum")
    print ("computing signal mean")
    print ("computing signal standard deviation")
    print ("computing variation and coeficients of variation")
    ...
    summary_var_lch = [Lsum_list,Lmean_list,Lsigma_list,Lvar_list, Lcof_var_list,top_ten_amp_LC_list,
                       spectral_centroid_Lch_list,spectral_flatness_lch_list,spectral_skewness_lch_list,lch_kurtosis_list, mfcc_var_lch_list ,energy_lchBsand_list]

    summary_var_rch = [Rsum_list,Rmean_list, Rsigma_list, Rvar_list, Lcof_var_list,top_ten_amp_RC_list,
                       spectral_centroid_Rch_list,spectral_flatness_rch_list,spectral_skewness_rch_list,rch_kurtosis_list,
                       mfcc_var_rch_list,energy_rchBand_list]
